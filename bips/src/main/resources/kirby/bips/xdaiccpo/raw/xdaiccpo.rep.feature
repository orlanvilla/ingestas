@bips
Feature: Tests for xdaiccpo in bips

  @execute
  Scenario: xdaiccpo-raw-rep
    Given an ingestion config file ${repository.endpoint}/${repository.repo}/kirby/co/bips/raw/xdaiccpo/${version}/xdaiccpo.rep.conf
    ...
    And the number of columns for output dataframe is more than to the number of columns for input dataframe
